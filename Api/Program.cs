...
builder.Services.AddSwaggerGen(options =>
{
    options.SwaggerDoc("v1",
        new OpenApiInfo
        { Title = "Customer Case Service API", Description = "API microservice for CRM Customer Case Service", Version = "v1" });
    options.ResolveConflictingActions(apiDescriptions => apiDescriptions.First());
    options.CustomSchemaIds(type => type.ToString().Replace(type.Namespace + ".", "").Replace("+", "."));
...
{
    options.JsonSerializerOptions.Converters.Add(new JsonStringEnumConverter(new UpperCaseEnumValueNamingPolicy()));
...
builder.Services.Configure<VaultSecretsConfiguration>(builder.Configuration.GetSection("VaultSecretsConfiguration"));
...
app.UseSerilogRequestLogging(opts =>
{
    ...
    opts.MessageTemplate = "HTTP {RequestMethod} {RequestPath} responded {StatusCode} in {Elapsed} ms";
}
);
...
app.UseSwagger(options =>
{
    ...
    {
        var basePath = "/api";
        doc.Servers = new List<OpenApiServer> { new OpenApiServer { Url = $"https://{req.Host.Value}{basePath}" } };
    });
});
app.UseSwaggerUI();
app.UsePathBase("/api");
