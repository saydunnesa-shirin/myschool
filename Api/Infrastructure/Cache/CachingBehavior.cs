  ...
  where TRequest : IRequest<TResponse> where TResponse : notnull
{
  ...
  private async Task<TResponse> GetResponseFromCacheOrExecute(ICacheableMediatrQuery cacheableQuery,
    RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
  {
    try
    {
      ...
      {
        _logger.LogDebug("Fetched from cache -> {cacheableQuery.CacheKey}",
    ...
    {
      _logger.LogError(ex, "Error retrieving data from cache for key -> {cacheableQuery.CacheKey}",
        cacheableQuery.CacheKey);
    }
  ...
  private async Task<TResponse> GetResponseAndAddToCache(ICacheableMediatrQuery cacheableQuery,
    RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
  {
    ...
    {
      _logger.LogError(ex, "Error storing data in cache for key -> {cacheableQuery.CacheKey}",
        cacheableQuery.CacheKey);
    }
